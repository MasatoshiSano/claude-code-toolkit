...
- ブランドアイデンティティとデザイン原則
- デザインシステムアーキテクチャ（リポジトリ構成、パッケージ情報）
- カラーパレット（5つのテーマ）、タイポグラフィ、レイアウトの詳細
- UIコンポーネントの使用方法と実装例
- アイコン（Serendie Symbols）とアニメーション
- Figma連携のワークフロー
- コード実装の原則とベストプラクティス
...
**主な特徴**:
- W3C Design Token Format Module準拠のデザイントークン
- 日本の伝統色ベースの5つのカラーテーマ（konjo、asagi、sumire、tsutsuji、kurikawa）
- 300+のSVGアイコン（Serendie Symbols）
- Panda CSSによるユーティリティファーストスタイリング
- Figma Code Connect対応
...
- **詳細な実装ガイド**: `.claude/commands/serendie-design.md`（パッケージ名、インストール手順、コード例など）
