...
Claude Code 用の **スキル** と **コマンド** と **環境構築ガイド** をまとめたツールキット。
...
| 🛠 スキル | [`skills/serendie-design`](skills/serendie-design/) | 三菱電機 Serendie Design System（@serendie/ui + @serendie/symbols + @serendie/design-token）で UI を実装するためのガイドライン。実プロジェクトから抽出した再利用可能なデザインパターン・トラブルシューティングを収録 |
| ⚡ コマンド | [`commands/task-flow.md`](commands/task-flow.md) | Team Task Scheduler MCP サーバーのツール群を使って、チーム／プロジェクト／タスク／マイルストーン等を自然言語で操作するスラッシュコマンド（要: Team Task Scheduler MCP） |
...
## serendie-design — Serendie Design System 実装ガイドライン

三菱電機の **Serendie Design System**（`@serendie/ui` + `@serendie/symbols` + `@serendie/design-token`）で UI を実装するための汎用スキル。公式リポジトリに基づくパッケージ名・全 33 コンポーネント・全 92 カラートークン・300+ アイコンを網羅し、実プロジェクトから抽出した再利用可能なデザインパターンを含む。

- **3 パターン提示ルール**: 新しいページ・セクション・コンポーネントの実装依頼では、必ず Pattern A（フラット型）／ B（カード型）／ C（サイドバー型）の 3 候補を提示してから実装に進む。
- **デザイン哲学**: 明るく風通しの良い配色、青はアクセント、白いカード、装飾的なグラデーション円など serendie.design の雰囲気を再現するための原則。
- **リファレンス分割**: カラートークン・コンポーネント一覧・アイコン・トラブルシューティングを `references/` に分離。
...
# ユーザーレベル（全プロジェクトで使う）
mkdir -p ~/.claude/skills
cp -r skills/serendie-design ~/.claude/skills/
...
依存: 対象プロジェクトに `npm install @serendie/ui @serendie/symbols @serendie/design-token`

### 使い方

Claude Code に「Serendie で〇〇ページを作って」「このUIをSerendieデザインに合わせて」等と頼むとスキルが起動する。実装前に必ず 3 パターンの候補が提示されるので、好きなパターンを選ぶ（または「まとめて実装」と指示）。

詳細は [`skills/serendie-design/SKILL.md`](skills/serendie-design/SKILL.md) と [`references/`](skills/serendie-design/references/)（`colors-tokens.md` / `components.md` / `icons.md` / `troubleshooting.md`）を参照。

---

# コマンド

## task-flow — Team Task Scheduler 操作コマンド

**Team Task Scheduler** MCP サーバーのツール群（チーム・ユーザー・プロジェクト・タスク・アイテム・マイルストーン・ベースライン・業務報告・コメント・タグ・依存関係・アクティビティログ）を、自然言語のリクエストから呼び出すための薄いラッパー型スラッシュコマンド。`$ARGUMENTS` でやりたいことを渡すと、Claude が適切な MCP ツールを順に実行して結果を報告する。

> ⚠️ **前提**: 別途「Team Task Scheduler」MCP サーバーを Claude Code に登録しておく必要がある（`team_list` / `project_list` / `task_create` 等のツールを提供するもの）。MCP が未登録だとこのコマンドは機能しない。
...
cp commands/task-flow.md ~/.claude/commands/
...
```text
/task-flow チームAの「新製品ライン構築」プロジェクトに来週分のタスクを3件追加して
/task-flow プロジェクト一覧と各プロジェクトの進行中タスク数を教えて
```

チームIDやプロジェクトIDが不明な場合は自動で `team_list` → `project_list` の順に確認し、更新・削除の前には対象を確認してから実行する。
